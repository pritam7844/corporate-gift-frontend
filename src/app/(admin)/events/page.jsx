...
import { Calendar, Plus, Tag, Search, CheckCircle2, X } from 'lucide-react';
import { useEvents } from '../../../hooks/useEvents';

export default function EventCatalog() {
  const { events, loading: eventsLoading, addEvent } = useEvents(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      setShowModal(false);
      setFormData({ name: '', startDate: '', endDate: '', isGlobal: true });
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden flex flex-col shadow-2xl">
            <div className="p-6 border-b flex justify-between items-center bg-gray-50">
              <h2 className="text-xl font-bold text-gray-800">New Event Template</h2>
            ...
            <form id="event-form" onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full border-2 p-3 rounded-xl focus:border-blue-500 outline-none font-sans"
                  value={formData.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center text-blue-600">
                    <Calendar size={14} className="mr-1" /> Default Start
                  </label>
                  <input
                    type="date" required
                    className="w-full border-2 p-3 rounded-xl outline-none font-sans text-sm"
                    value={formData.startDate}
                    onChange={(e) => setFormData({ ...formData, startDate: e.target.value })}
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center text-red-600">
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-8 py-2.5 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-all disabled:bg-blue-300 shadow-blue-200"
                >
      ...
      {eventsLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {events.map((event) => (
            <div key={event._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 group">
              ...
              <div className="flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button className="flex-1 py-2 text-sm font-bold text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-600 hover:text-white transition-all">
                  Edit Template
                </button>
              </div>
            </div>
          ...
          {events.length === 0 && (
            <div className="col-span-full py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-center">
              ...
              <button onClick={() => setShowModal(true)} className="mt-2 text-blue-600 font-bold hover:underline font-sans">
